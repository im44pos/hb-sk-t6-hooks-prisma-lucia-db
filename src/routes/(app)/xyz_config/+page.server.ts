...
export const load: PageServerLoad = async ({ locals }) => {
	// console.log("PageServerLoad @ src/routes/(app)/xyz_config/+page.server.ts")
	const session = await locals.auth.validate();
...
export const actions: Actions = {
	logout: async ({ locals }) => {
		// console.log("Actions @ src/routes/(app)/xyz_config/+page.server.ts")
		const session = await locals.auth.validate();
		if (!session) return fail(400);
		await auth.invalidateSession(session.sessionId); // invalidate session
		locals.auth.setSession(null); // remove session cookie
		// console.log("Logout @ src/routes/(app)/xyz_config/+page.server.ts session cookie is now deleted.")
		throw redirect(302, "/login"); // redirect to login page
	},
}